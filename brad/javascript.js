...
function GetPlayerEmbed(o, c) {
	...
	case 'angels':
		...
		<p style="font-size:20px>Try a different server if you're lagging</p>
...
function setServer(o, c, s) {
	...
	if(o == "angels"){
		...
		return `<div id="player"></div><script>var player = new Clappr.Player({source: "${c}", mute: true, autoPlay: true, parentId: "#player", height: ${h}, width: "100%",events: {onError: function(e) {setTimeout(function() { player.configure(player.options); }, 30000);}}});</script>`;
	}
}
